...
//
// Platform note: this uses QtBluetooth's central role, which is fine on **Android + desktop (BlueZ)**.
// On **iOS** qz runs mostly peripheral-mode via a native Swift wrapper, so a native central bridge (like
// zwiftclickremote's `iOS_zwiftClickRemote`) is a follow-up — this class is compiled non-iOS for now.
//
